...
def find_dir_with_numberprefix(name_without_numberprefix, root_dir, prefix_separator='_'):
    ...
    if match_dir:
        ...
        return os.path.join(ab_root_dir, f"{max_no + 1:02}{prefix_separator}{name_without_numberprefix}/")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="動画作成用のスライドプロジェクトを作成しvscodeで開く。もし既に存在するプロジェクトの場合はそのプロジェクトをvscodeで開く。")
    ap.add_argument("project_name", metavar="PROJECT_NAME", type=str, help="プロジェクト名")
    ap.add_argument("--root-dir", type=str, default=DEFAULT_ROOT_DIR,
                    help=f"スライドプロジェクトのルートディレクトリ。(デフォルト値: {DEFAULT_ROOT_DIR})")
    ap.add_argument("--with-code", dest="with_code", default=True, action='store_true', help="VScodeでプロジェクトを開く。")
    ap.add_argument("--without-code", dest="with_code", action='store_false', help="VScodeでプロジェクトを開かない。")
